...
DROP TABLE IF EXISTS hv_orc_onboard_owner_agg_temp;

CREATE TABLE IF NOT EXISTS hv_orc_onboard_owner_agg_temp
...
,   COUNT(onboardpoltime) AS onboardcontainercount
,   COUNT(dwellatpol) AS dwelltimeatpolcontainercount
,   COUNT(case when dwellatpol > 0 THEN dwellatpol ELSE NULL END) AS positivedwelltimeatpolcontainercount
,   COUNT(case when dwellatpol < 0 THEN dwellatpol ELSE NULL END) AS negativedwelltimeatpolcontainercount
,   SUM(dwellatpol) AS dwelltimeatpoldayssum
,   SUM(CASE WHEN dwellatpol > 0 THEN dwellatpol ELSE NULL END) AS positivedwelltimeatpoldayssum
,   SUM(CASE WHEN dwellatpol < 0 THEN dwellatpol ELSE NULL END) AS negativedwelltimeatpoldayssum
,   MIN(CASE WHEN dwellatpol > 0 THEN dwellatpol ELSE NULL END) AS minpositivedwelltimeatpoldays
,   MAX(CASE WHEN dwellatpol > 0 THEN dwellatpol ELSE NULL END) AS maxpositivedwelltimeatpoldays
FROM hv_orc_tt_vmcontainermove_owner_base AS a
    INNER JOIN dimension_common.hv_ext_tt_dw_date_dim AS b
    ON b.the_date = to_date(a.onboardpoltime)
...
DROP TABLE IF EXISTS hv_orc_onboard_owner_agg;

ALTER TABLE hv_orc_onboard_owner_agg_temp RENAME TO hv_orc_onboard_owner_agg;