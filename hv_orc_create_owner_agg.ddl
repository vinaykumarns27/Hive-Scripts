...
DROP TABLE IF EXISTS hv_orc_create_owner_agg_temp;

CREATE TABLE IF NOT EXISTS hv_orc_create_owner_agg_temp
...
,   COUNT(createtime) AS createdatecontainercount
,   COUNT(firstetd) AS firstetdpolcompletenesscontainercount
,   COUNT(firsteta) AS firstetapodcompletenesscontainercount
FROM hv_orc_tt_vmcontainermove_owner_base AS a
    INNER JOIN dimension_common.hv_ext_tt_dw_date_dim AS b
    ON b.the_date = to_date(a.createtime)
...
DROP TABLE IF EXISTS hv_orc_create_owner_agg;

ALTER TABLE hv_orc_create_owner_agg_temp RENAME TO hv_orc_create_owner_agg;